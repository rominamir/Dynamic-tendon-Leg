...
from Basde_Code import DynamicLegEnv_base as  DynamicLegEnv
...
folder = "test_Monotonic_10k_jan14"
...
def train_env(seed_value, train=True):
    if train:
        print(f"Starting training with seed {seed_value}")
        #env = DynamicLegEnv(render_mode=None)  # Disable rendering during training
        # Ensure a max episode length of 500 steps
        env = DynamicLegEnv(render_mode=None)
    else:
        print(f"Starting evaluation with seed {seed_value}")
        #env = DynamicLegEnv(render_mode="human")  # Enable rendering during evaluation
        env = TimeLimit(DynamicLegEnv(render_mode=None), max_episode_steps=500)
    ...
    if train:
        ...
        model.save(f"PPO_seed_Dynamic_{seed_value}.model")
        ...
        # Load model and evaluate with rendering
...
if __name__ == '__main__':
    ...
    #train = input("Enter '♦train' or 'eval' for mode: ") == "t,rain"
...
import numpy as np
from stable_baselines3 import PPO
from Basde_Code import DynamicLegEnv
import multiprocessing as mp
from stable_baselines3.common.callbacks import BaseCallback
from gymnasium.wrappers import RecordVideo
import os
...
def linear_entropy(initial_value):
    def func(progress_remaining):
        return progress_remaining * initial_value
    return func
...
def evaluate_model(seed_value):
    print(f"Starting evaluation with seed {seed_value}")
    env = DynamicLegEnv(render_mode="rgb_array")  # Use 'rgb_array' for video recording
    env = RecordVideo(
        env,
        video_folder=f"./videos/seed_{seed_value}/",  # Folder to save videos
        episode_trigger=lambda episode_id: (episode_id % 10) == 0,  # Record every episode
    )

    
    env.seed(seed_value)
    print("Eval env observation_space =", env.observation_space)
    model = PPO.load(f"./tensorboard_log/model/{folder}/PPO_seed_{seed_value}.model", env= None)
    model.set_env(env)  # manually set after loading
...
def train_env(seed_value, train=True, fps=30):
    if train:
        ...
        env = DynamicLegEnv(render_mode=None)
        env.seed(seed_value)
        print("Train env observation_space =", env.observation_space)

        model = PPO(
            'MlpPolicy', env, verbose=1, seed=seed_value,
            tensorboard_log=f"./tensorboard_log/{folder}/ppo_seed{seed_value}",
            ent_coef=linear_entropy(0.05),  
            gamma=0.9, learning_rate=0.0003
        )
...
def train_env_(seed_value, train=True):
    if train:
        ...
        env = DynamicLegEnv(render_mode="rgb_array")
        env.seed(seed_value)

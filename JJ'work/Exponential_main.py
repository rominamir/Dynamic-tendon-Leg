...
from Expo import ExpoLegEnv  # Import the environment
...
def evaluate_model(seed_value, growth_factor=0.01):
    ...
    model_path = f"./tensorboard_log/model/{folder}/PPO_seed_{seed_value}.model"
...
def train_env(seed_value, train=True, growth_factor=0.01):
    """Trains the PPO model in ExpoLegEnv and records stiffness/distance history."""
    if train:
        print(f"Starting training with seed {seed_value} and growth_factor {growth_factor}")
        env = ExpoLegEnv(render_mode=None, growth_factor=growth_factor)  # Adjustable growth factor
        env.seed(seed_value)
...
if __name__ == '__main__':
    print("This is an Exponential function run")

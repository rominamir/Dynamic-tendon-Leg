...
plt.title("PPO Training Reward Curve for Different Stiffness Schedules")
plt.legend()
plt.grid()
plt.show()
...
plt.xlabel("Training Steps")
plt.ylabel("Reward Value")
plt.title("A2C Training Reward Curve for Different Stiffness Schedules")
plt.legend()
plt.grid()
plt.show()

# ------------- 绘制 PPO 和 A2C 训练奖励曲线在同一张图上 -------------
plt.figure(figsize=(12, 6))

# 绘制 PPO 训练曲线（使用虚线）
for key in ["ppo_log", "ppo_linear", "ppo_exp", "ppo_constant"]:
    strategy = key.split("_")[1]  # 提取策略名称
    plt.plot(dataframes[key]["Step"], dataframes[key]["Value"],
             label=f"PPO {strategy}", linestyle='dashed', color=colors[strategy])

# 绘制 A2C 训练曲线（使用实线）
for key in ["a2c_log", "a2c_linear", "a2c_exp", "a2c_constant"]:
    strategy = key.split("_")[1]  # 提取策略名称
    plt.plot(dataframes[key]["Step"], dataframes[key]["Value"],
             label=f"A2C {strategy}", linestyle='solid', color=colors[strategy])

plt.xlabel("Training Steps")
plt.ylabel("Reward Value")
plt.title("PPO vs A2C Training Reward Curve for Different Stiffness Schedules")
plt.legend()
plt.grid()
plt.show()
